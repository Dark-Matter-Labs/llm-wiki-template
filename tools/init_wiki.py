...
def check(root=None) -> Report:
    ...
    # What it READS. Absent means the two directions are the same, which is true of every
    # wiki written before the fields were split.
    downs = list(fed["reads_from"]) if "reads_from" in fed else list(ups)
    repo_dir = root.name
    ...
    if name in downs:
        r.fail(f"`reads_from` contains this wiki's own name (`{name}`) — sync_commons.py "
    ...
    if role == "spoke" and not ups and not downs:
        ...
        r.ok(f"one-way spoke: reads {', '.join(downs)} and contributes to nothing. "
             f"contribute.py will refuse every target, which is the point.")
    elif ups:
    ...
    readonly = [c for c in downs if c not in ups]
    if readonly:
        r.ok(f"reads without contributing to: {', '.join(readonly)} — a one-way link, "
             f"enforced by contribute.py refusing them by name")

    known = known_federation(root)
    if known:
        unknown = [u for u in ups + downs if u not in known]
        if unknown:
            r.fail(f"`contributes_to`/`reads_from` names {', '.join(unknown)}, which the federation "
...
def render_federation(name, role, display_name, ups, downs=None) -> dict:
    fed = {
        ...
        "role": role,
        "contributes_to": ups,
        "_note": "Generated by tools/init_wiki.py. `contributes_to` is the UP flow: the "
                 "commons contribute.py may open a pull request against. `reads_from` is the "
                 "DOWN flow that sync-commons.yml clones into .commons/<name>/export; when it "
                 "is absent the two are the same, which is true of most wikis.",
        "pre_split_months": [],
    ...
    if downs is not None and sorted(downs) != sorted(ups):
        # Written only when the two directions genuinely differ, so the ordinary case stays
        # one field and the exception is visible as an exception.
        fed["reads_from"] = list(downs)
        fed["_reads_note"] = ("This wiki READS these and may not contribute to them. "
                              "contribute.py refuses any target not in `contributes_to`, "
                              "naming this field when it does. A one-way link is a decision "
                              "about what may leave this repository; changing it is a "
                              "person's call, not a convenience.")
    return fed
...
def init(name, role, display_name, ups, downs=None, force=False, root=None) -> tuple[int, list[str]]:
    root = root or ROOT
    written, refused = [], []
    ...
    for rel, payload in ((FEDERATION, render_federation(name, role, display_name, ups, downs)),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--reads-from", dest="downs", default=None,
                    help="commons to clone DOWN, comma separated. Omit when the same as "
                         "--contributes-to. Give it alone for a wiki that reads but never "
                         "writes, which is how a repo holding material that must not travel "
                         "still sees what the group knows.")
    ...
    if args.init:
        ...
        downs = ([d.strip() for d in args.downs.split(",") if d.strip()]
                 if args.downs is not None else None)
        code, lines = init(args.name, args.role, args.display_name, ups,
                           downs=downs, force=args.force)
        for l in lines:
            print(f"  {l}")
        if code:
            return code
