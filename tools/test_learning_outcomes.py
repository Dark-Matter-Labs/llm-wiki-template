...
def main():
    ...
    # THE BRIEF: the shareable form, and after 2026-09-10 the CONTENT is the product.
    # The funder cut must name no internal page in any format; the meta layer (reflections,
    # deltas, metacognition) must not appear as "what we came to think", because that is the
    # metadata-about-the-wiki the rewrite exists to remove; and a page's own description must
    # be what gets shown.
    with tempfile.TemporaryDirectory() as t:
        r = Repo(pathlib.Path(t))
        r.write("seed", page("Seed", visibility="public"))
        r.commit(T0)
        r.write("syn", page("A Real Synthesis", type="synthesis", visibility="public",
                            body="body\n"))
        r.write("reflections/2026-W33", page("Weekly reflection — 2026-W33", type="synthesis",
                                             visibility="public"))
        r.write("opt", page("Securing Cities (OPT-020)", type="entity", visibility="public"))
        r.write("int", page("An Internal Concept", type="concept", visibility="internal"))
        r.write("challenges/joker-x", page("Joker: the other side", type="synthesis",
                                           visibility="public"))
        r.commit(T0 + 20 * DAY)
        r.activate()
        since, until = day(T0 + 10 * DAY), day(T0 + 30 * DAY)
        f, i = lo.build(since, until, "funder"), lo.build(since, until, "internal")
        secs = {x["lead"]: x for x in lo.narrative(i)["sections"]}

        check("a synthesis appears under what we came to think",
              [x["title"] for x in secs["What we came to think"]["items"]] == ["A Real Synthesis"],
              str(secs["What we came to think"]["items"]))
        check("a weekly reflection does NOT — it is about the wiki, not the work",
              all("reflection" not in x["title"] for s2 in secs.values() for x in s2["items"]))
        check("a joker challenge gets its own section, not the position section",
              [x["title"] for x in secs["What we argued against ourselves"]["items"]]
              == ["Joker: the other side"])
        check("an OPT-coded page lands in the portfolio section",
              [x["title"] for x in secs["What moved in the portfolio"]["items"]]
              == ["Securing Cities (OPT-020)"])
        check("the item carries the page's OWN description, not a count",
              secs["What moved in the portfolio"]["items"][0]["detail"] == "d")
        check("an empty section says which nothing, rather than going blank",
              secs["What we stopped believing"]["items"] == []
              and "No position was retired" in secs["What we stopped believing"]["empty"])

        for fmt, fn in (("text", lo.render_text), ("markdown", lo.render_markdown),
                        ("slack", lo.render_slack), ("html", lo.render_html)):
            out = fn(f)
            check(f"the funder brief names no internal page — {fmt}", "An Internal Concept" not in out)
        ...
        check("the counts are demoted to assembly, not the lede",
              "pages dated in the window" in dict(lo.narrative(i)["assembly"])
              and "pages entered" not in lo.narrative(i)["lede"])
