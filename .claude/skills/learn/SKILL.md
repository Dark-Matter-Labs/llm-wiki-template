---
name: learn
description: Turn a verified recurring pattern into a new skill. **Fires automatically** when the same correction, question or workaround appears a third time — in the log, in a session, or across PRs. Three occurrences is the threshold; waiting to be told costs the two that already happened. Use when Indy says "make that a skill" or "learn this pattern", or when metacognition (or any session) spots a recurring pattern worth reinforcing. Always asks Indy to verify the pattern before creating anything.
---
...
2. **Ask Indy to verify — mandatory:**
...
5. **Log it:** `## [YYYY-MM-DD] learn | <skill name> verified by Indy`.

## Hard rules
- **Never create or modify a skill without the explicit verification step.** Not even a "small" one.
- **Never mark a skill as verified when it wasn't.** The log line "verified by Indy" may only be
  written after a real yes from Indy. If he tweaks the pattern, verify the tweaked version.
- If Indy says no, that's a useful result — note in-session what was rejected and why (don't file a page).
